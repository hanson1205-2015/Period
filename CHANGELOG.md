...
- Execution backends unified: Period now runs all programs through a single bytecode VM path. The old Cranelift JIT and tree-walk fallback have been removed, so `PERIOD_NO_JIT` and `PERIOD_NO_BYTECODE` are no longer used. The `.github/scripts/run_paths.sh` compatibility wrapper still runs every example to guard against regressions.
